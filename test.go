...
func qualifyLock(index int, key int) (r1 uintptr, r2 uintptr, err syscall.Errno) {
...
func semLock(index int, semid int) int {
	...
	if int(r1) < 0 {
		log.Printf("线程%d error:semLock error is %v,%v,%v\n", index, r1, r2, err)
	}
	return int(r1)
}
...
func worker(index int, startSignal <-chan struct{}, wg *sync.WaitGroup, semid int) {
	defer wg.Done()
	<-startSignal              // 等待启动信号
	id := semget(index, semid) // 在接收到启动信号后调用 semget
	fmt.Printf("线程%d 的semid:%d 对应的信号量值为%d, lock信号量结果: %d\n", index, id, semShow(index, id), semLock(index, id))
}
